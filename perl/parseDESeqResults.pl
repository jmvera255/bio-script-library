...
my (%opts);
getopts("b:l:m:p:q:ud", \%opts);

my $usage = "********
Usage: parseDESeqResults.pl <options> <DESeq.txt> > STDOUT
********

DESeq results file must have a header starting with Gene

Where one of the following options must be selected:
	-b <int>   returns genes with base mean >= int)
	-l <int>   returns genes with log2 fold change cutoff (shrunken values)
		   default selects all genes >= and <= int
